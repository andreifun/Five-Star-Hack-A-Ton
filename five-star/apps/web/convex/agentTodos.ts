...
export const listByThread = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!thread || thread.businessId !== args.businessId) throw new Error("Thread not found");
    return await ctx.db
      .query("agentTodos")
...
export const createForTip = mutation({
  ...
  handler: async (ctx, args) => {
    await requireBusinessOwner(ctx, args.businessId);
    const [tip, thread] = await Promise.all([ctx.db.get(args.tipId), ctx.db.get(args.threadId)]);
    if (!tip || tip.businessId !== args.businessId || !thread || thread.tipId !== args.tipId) {
