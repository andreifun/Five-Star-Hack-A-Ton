...
export const claimKickoff = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!thread) throw new Error("Thread not found");
    await requireBusinessOwner(ctx, thread.businessId);
    if (!thread.tipId || thread.messageCount > 0 || (thread.kickoffStatus !== undefined && thread.kickoffStatus !== "pending" && thread.kickoffStatus !== "failed")) {
      return false;
    }
    await ctx.db.patch(args.threadId, { kickoffStatus: "running", kickoffError: undefined });
    return true;
  },
});
...
export const finishKickoff = internalMutation({
  ...
  handler: async (ctx, args) => {
    await ctx.db.patch(args.threadId, { kickoffStatus: args.status, kickoffError: args.error });
  },
});
