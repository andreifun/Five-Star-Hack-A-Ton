...
import { Check, ExternalLink, FileSearch, Mail, Send } from "lucide-react"
...
export function ConversationArtifacts({
  ...
  const reports =
    useQuery(api.researchReports.listByTip, { businessId, tipId }) ?? []
  const drafts =
    useQuery(api.emailDrafts.listByTip, { businessId, tipId }) ?? []
  const approveDraft = useMutation(api.emailDrafts.approve)
  const sendDraft = useAction(api.ai.email.send)

  if (reports.length === 0 && drafts.length === 0) return null
        ...
        <article key={draft._id} className="rounded-xl border bg-card p-4">
          ...
          <div className="mt-3 space-y-2 text-sm">
            <p>
              ...
              {draft.recipients.join(", ") || "No recipients"}
            </p>
            <p>
          ...
          <div className="mt-3 flex gap-2">
            ...
            {draft.status === "approved" ? (
              <Button
                size="sm"
                onClick={() => sendDraft({ draftId: draft._id })}
              >
